    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body p-0 py-3">
            <form action="{{ url('share/file/'. $r->route) }}" method="post" class="form form-inline col-sm-12" autocomplete="off">
                {{ method_field('PUT') }}
                {{ csrf_field() }}
          ...
          @if($r->sharedFile != null && $r->sharedFile->count() > 0)
                  ...
                  <ul class="navbar-nav ml-auto px-0">
                    <li class="nav-item dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-right text-right">
                        @if($t->access == 'view')
                          <a href="{{ url('edit/file/share/'.$t->file->route.'/'.$t->id_user.'/edit') }}" class="dropdown-item">
                            Edit
                          </a>                            
                        @else
                        <a href="{{ url('edit/file/share/'.$t->file->route.'/'.$t->id_user.'/view') }}" class="dropdown-item">
                            View
                          </a>
                        @endif
                        <div class="dropdown-divider"></div>
                        <a href="{{ url('delete/file/share/'.$t->file->route.'/'.$t->id_user) }}" class="dropdown-item text-danger">
                          Remove Access
                        </a>
                      </div>
                    </li>
